...
module Rom_Sel_Reader (
    ...
    always_comb begin
        case (music_sel)
            2'b00:   begin 
                song_rom_sel = song0_rom;
                rom_cnt = 6'd42;
            end
            2'b01:   begin 
                song_rom_sel = song1_rom;
                rom_cnt = 6'd42;   //rom 개수 따라 수정
            end
            2'b10:   begin
                song_rom_sel = song2_rom;
                rom_cnt = 6'd42;   //rom 개수 따라 수정
            end
            default: begin
                song_rom_sel = song0_rom;
        ...
        if (reset) begin
            ...
            if (main_state == 3'b011) begin
                if (note_done) begin     // 타이머에 맞춰 노트가 발생할 때만 ROM 주소 증가
                    if (rom_addr == rom_cnt) rom_addr <= 6'd0;  // 반복 재생
                    else rom_addr <= rom_addr + 1'b1;
                end
            end else if(main_state == 3'b101) begin     // done에서 rom 주소 0으로 reset 
                rom_addr <= 0;
            end
        end
    end
