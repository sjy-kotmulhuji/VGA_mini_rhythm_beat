...
class RhythmGame:
    ...
    def __init__(self):
        ...
        self.screens = {
            ...
            "game":   GameScreen(self.screen, self.clock, self.uart),
    ...
    def _switch_screen(self, screen_name):
        ...
        if screen_name in self.screens:
            ...
            # 게임 화면 진입 시 상태 초기화 (곡 데이터 재로드 포함)
            if screen_name == "game" and hasattr(self.current_screen, "reset"):
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # 시작 화면에서 ESC -> 프로그램 종료
                    if event.key == pygame.K_ESCAPE and self.current_screen_name == "start":
                        self.running = False
                    else:
                        # 현재 화면에 이벤트 전달
            ...
            while trigger is not None:
                print(f"[UART] 트리거 수신: 0x{trigger:02X}")
