...
module GameResult (
    ...
    */
    ...
    // ===============================================================
    logic [SLOT_COUNT-1:0] slot_in_zone;
    logic [3:0] lane_has_note;
    integer slot_idx;

    always_comb begin
        slot_in_zone  = '0;
        lane_has_note = 4'b0000;

        for (slot_idx = 0; slot_idx < SLOT_COUNT; slot_idx = slot_idx + 1) begin
            slot_in_zone[slot_idx] =
                (lcnt[slot_idx] >= JUDGE_LINE - JUDGE_GOOD_HALF) &&
                (lcnt[slot_idx] <= JUDGE_LINE + JUDGE_GOOD_HALF);

            if (slot_in_zone[slot_idx])
                lane_has_note = lane_has_note | pos[slot_idx];
        end
    end
    ...
    // ===============================================================
    wire lane0_has_note = lane_has_note[0];
    wire lane1_has_note = lane_has_note[1];
    wire lane2_has_note = lane_has_note[2];
    wire lane3_has_note = lane_has_note[3];
    ...
    assign miss = input_miss | (|miss_slot);
...
module slot_fsm #(
    ...
    parameter int PERFECT_HALF = 10,  // perfect 반폭 -> 410~430
    parameter int GOOD_HALF = 20  // 판정존 반폭 -> 400~440
) (
    input logic clk,
    input logic reset,
