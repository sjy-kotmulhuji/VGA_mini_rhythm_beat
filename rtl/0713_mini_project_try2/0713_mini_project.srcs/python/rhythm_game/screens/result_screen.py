...
class ResultScreen(BaseScreen):
    ...
    def handle_trigger(self, event):
        if isinstance(event, dict) and event.get("down"):
            self.next_screen = "start"
    ...
    def draw(self):
        ...
        combo_surf = self.font_combo.render(f"MAX COMBO: {self.max_combo}", True, COLOR_NEON_PINK)
        self.screen.blit(combo_surf, combo_surf.get_rect(center=(cx, int(self.sh * 0.38))))
        ...
        for i, entry in enumerate(self.leaderboard_data):
            ...
            rank_text = f"#{i+1}"
            date_text = entry.get("date", "Unknown")[2:10] # YY-MM-DD
            score_text = f"{entry.get('score', 0):07d}"
            combo_text = f"[{entry.get('max_combo', 0)}x]"
            
            # 가상의 캡쳐 이미지 공간 (사각형)
            img_rect = pygame.Rect(cx - 200, y - 10, 20, 20)
            pygame.draw.rect(self.screen, color, img_rect, 1)
            
            # 텍스트 렌더링
            r_surf = self.font_rank.render(rank_text, True, color)
            d_surf = self.font_rank.render(date_text, True, COLOR_WHITE)
            s_surf = self.font_rank.render(score_text, True, COLOR_NEON_CYAN)
            c_surf = self.font_rank.render(combo_text, True, COLOR_NEON_PINK)
            
            self.screen.blit(r_surf, (cx - 250, y - r_surf.get_height()//2))
            self.screen.blit(d_surf, (cx - 160, y - d_surf.get_height()//2))
            self.screen.blit(s_surf, (cx + 50, y - s_surf.get_height()//2))
            self.screen.blit(c_surf, (cx + 170, y - c_surf.get_height()//2))
